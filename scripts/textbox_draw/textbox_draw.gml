/// @description  textbox_draw(id, x1, y1, x2, y2)
...

{
    ...
    deletetext=0
    inserttext=""
    lineheight=string_height(string_hash_to_newline(" "))
    ...
    if (textbox_focus=i) {
        ...
        if (key_press[vk_right] || key_press[vk_left] || (key_press[vk_up] && textbox_select_mouseline>0) || (key_press[vk_down] && textbox_select_mouseline<i.lines-1)) {  //Arrow keys to move marker
            ...
            if (!i.single_line && (key_press[vk_up] || key_press[vk_down])) {  // Move marker up/down
                var currentx,nextx;
                currentx=string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[textbox_select_mouseline],1,textbox_select_mousepos),"#","\#")))
                nextx=0
                textbox_select_mouseline+=key_press[vk_down]-key_press[vk_up]
                for (textbox_select_mousepos=0; textbox_select_mousepos<=string_length(i.line[textbox_select_mouseline]); textbox_select_mousepos++) { // Find correct position
                    nextx+=string_width(string_hash_to_newline(string_replace(string_char_at(i.line[textbox_select_mouseline],textbox_select_mousepos),"#","\#")))
        ...
        if (deletetext!=0 || inserttext!="") {
            ...
            for (l=0; l<textbox_select_mouseline; l++) realmousepos+=string_length(i.line[l])+(!i.line_wrap[l+1] && !i.line_single[l])
            for (l=0; l<textbox_select_startline; l++) realstartpos+=string_length(i.line[l])+(!i.line_wrap[l+1] && !i.line_single[l])
            for (l=0; l<textbox_select_endline; l++) realendpos+=string_length(i.line[l])+(!i.line_wrap[l+1] && !i.line_single[l])
            ...
            if (textbox_select_startline!=textbox_select_endline || textbox_select_startpos!=textbox_select_endpos) {  //Several characters
                if (textbox_select_startline=textbox_select_endline) {  // Same line, just do string_delete
                    ...
                    for (l=textbox_select_startline+1; l<i.lines; l++) {
        ...
        if (i.max_chars>0) {  // Check max limit
            var maxlen=i.max_chars-string_length(i.text);
        ...
        if (inserttext!="") {
            ...
            if (string_pos(chr(10),inserttext)>0) {  // Add text with multiple lines (Paste or linebreak)
                ...
                i.lines+=b
                for (l=i.lines-1; l>=textbox_select_mouseline+b; l--) {  //Push up
                    i.line[l]=i.line[l-b]
        ...
        if (textbox_select=i) {  // Move up/down if dragging outside of box
            textbox_marker=current_time
            if (i.single_line) {
                if (mouse_x<xx) i.start--
                if (mouse_x>xx+w) i.start++
            } else {
                if (mouse_y<yy) i.start--
                if (mouse_y>yy+h) i.start++
            }
        }
        if (textbox_click>0) {
    ...
    if (i.single_line) {  // Calculate the amount of characters visible
        ...
        ww=0
        for (a=string_length(i.line[0]); a>=0; a--) {
            ww+=string_width(string_hash_to_newline(string_replace(string_char_at(i.line[0],a),"#","\#")))
            b=a
            if (ww>w) break
        }
        ...
        ww=0
        i.chars=0
        for (a=i.start+1; a<=string_length(i.line[0]); a++) {
            ww+=string_width(string_hash_to_newline(string_replace(string_char_at(i.line[0],a),"#","\#")))
            if (ww>w) break
            i.chars++
        }
        ...
        if (changetext || i.last_width!=w || inserttext!="" || deletetext!=0) {  // Detect box width or line length changes.
            for (l=1; l<i.lines; l++) {  // Move words up?
                ww=string_width(string_hash_to_newline(string_replace_all(i.line[l-1],"#","\#")))
                if (!i.line_wrap[l] || ww>w) continue
                if (i.line_single[l-1]) {  // Single-worded line
                    for (p=1; p<=string_length(i.line[l]); p++) {  // Try to add remaining letters
                        if (ww+string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[l],1,p),"#","\#")))>w) break
                    ...
                    if (textbox_select_mouseline=l && textbox_select_mousepos<=p) {textbox_select_mouseline-- textbox_select_mousepos+=a}
                    if (textbox_select_clickline=l && textbox_select_clickpos<=p) {textbox_select_clickline-- textbox_select_clickpos+=a}
                    if (textbox_select_endline=l && textbox_select_endpos<=p) {textbox_select_endline-- textbox_select_endpos+=a}
                    if (textbox_select_startline=l && textbox_select_startpos<=p) {textbox_select_startline-- textbox_select_startpos+=a}
                ...
                while (i.line[l]!="") {  // Try to add words
                    ...
                    if (ww+string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[l],1,p-1),"#","\#")))>w) break
                    a=string_length(i.line[l-1])  // Move markers if affected
                    if (textbox_select_mouseline=l && textbox_select_mousepos<=p) {textbox_select_mouseline-- textbox_select_mousepos+=a}
                    if (textbox_select_clickline=l && textbox_select_clickpos<=p) {textbox_select_clickline-- textbox_select_clickpos+=a}
                    if (textbox_select_endline=l && textbox_select_endpos<=p) {textbox_select_endline-- textbox_select_endpos+=a}
                ...
                if (i.line[l]="") { // Remove empty line
                    ...
                    for (a=l; a<i.lines; a++) {
                        ...
                        if (textbox_select_mouseline=a+1) textbox_select_mouseline--  // Move markers if affected
                        if (textbox_select_clickline=a+1) textbox_select_clickline--
                        if (textbox_select_endline=a+1) textbox_select_endline--
                        if (textbox_select_startline=a+1) textbox_select_startline--
                    }
                    l--
                }
            }
            for (l=0; l<i.lines; l++) {  // Move words down?
                if (string_width(string_hash_to_newline(string_replace_all(i.line[l],"#","\#")))>w) {
                    i.line_single[l]=false
                    for (p=string_length(i.line[l]); p>1; p--) {  // Look for words
                        a=string_char_at(i.line[l],p)
                        if (a=" " || a="-") {
                            if (string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[l],1,p-1),"#","\#")))<w) break
                        }
                    }
                    if (p=1) {  // Single-word line found
                        i.line_single[l]=true
                        for (p=string_length(i.line[l])-1; p>1; p--) {
                            if (string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[l],1,p),"#","\#")))<w) break
                        }
                    }
                    ...
                    if (a) {  // Create new line for wrapped text
                        for (b=i.lines; b>l; b--) {  // Push
                            i.line[b]=i.line[b-1]
                            i.line_wrap[b]=i.line_wrap[b-1]
                            i.line_single[b]=i.line_single[b-1]
                            if (textbox_select_mouseline=b) textbox_select_mouseline++  //Move markers if affected
                            if (textbox_select_clickline=b) textbox_select_clickline++
                            if (textbox_select_endline=b) textbox_select_endline++
                            if (textbox_select_startline=b) textbox_select_startline++
                        }
                        i.lines++
                    ...
                    if (textbox_select_mouseline=l && textbox_select_mousepos>=p) {textbox_select_mouseline++ textbox_select_mousepos-=p}
                    if (textbox_select_clickline=l && textbox_select_clickpos>=p) {textbox_select_clickline++ textbox_select_clickpos-=p}
                    if (textbox_select_endline=l && textbox_select_endpos>=p) {textbox_select_endline++ textbox_select_endpos-=p}
                    if (textbox_select_startline=l && textbox_select_startpos>=p) {textbox_select_startline++ textbox_select_startpos-=p}
    ...
    for (l=i.start*!i.single_line; l<i.lines; l++) {
        var ly=(l-i.start*!i.single_line)*lineheight;  // Current line y value
            ...
            if ((mouse_x>=xx || textbox_select=i) &&
                ...
                if (mouse_check_button(mb_left)) {
                    if (i.select_on_focus && textbox_lastfocus!=i) { // Select all
                        ...
                        textbox_select=i
                        ww=0
                        for (a=i.start*i.single_line; a<string_length(i.line[l]); a++) {  // Find character over mouse
                            b=string_width(string_hash_to_newline(string_replace(string_char_at(i.line[l],a+1),"#","\#")))
                            ww+=b
        ...
        if (i.single_line) {
            if (textbox_focus=i && textbox_select_startpos!=textbox_select_endpos) {
                ...
                for (a=0; a<3; a++) str[a]=string_replace_all(str[a],"#","\#")
                if (str[0]!="") {  // Text before or outside selection
                    draw_set_color(color_normal)
                    if( global.enabledText ) draw_text(xx,yy,string_hash_to_newline(str[0]))
                }
                if (str[1]!="") {  // Selected text
                    draw_set_color(color_selection)
                    draw_rectangle(min(xx+w,xx+string_width(string_hash_to_newline(str[0]))),yy,min(xx+w,xx+string_width(string_hash_to_newline(str[0]+str[1]))),yy+lineheight - 1,0)
                    draw_set_color(color_selected)
                    if( global.enabledText ) draw_text(xx+string_width(string_hash_to_newline(str[0])),yy,string_hash_to_newline(str[1]))
                }
                if (str[2]!="") {  // Text after selection
                    draw_set_color(color_normal)
                    if( global.enabledText ) draw_text(xx+string_width(string_hash_to_newline(str[0]+str[1])),yy,string_hash_to_newline(str[2]))
                }
            } else { // Unselected
                draw_set_color(color_normal)
                if( global.enabledText ) draw_text(xx,yy,string_hash_to_newline(string_replace_all(string_copy(i.line[0],i.start+1,i.chars),"#","\#")))
            }
        } else {
            if (textbox_focus=i && (textbox_select_startline!=textbox_select_endline || textbox_select_startpos!=textbox_select_endpos)) {  // This line is selected
                ...
                if (str[0]!="") {  // Text before or outside selection
                    draw_set_color(color_normal)
                    if( global.enabledText ) draw_text(xx,yy+ly,string_hash_to_newline(str[0]))
                }
                if (str[1]!="") {  // Selected text
                    draw_set_color(color_selection)
                    draw_rectangle(min(xx+w,xx+string_width(string_hash_to_newline(str[0]))),yy+ly,min(xx+w,xx+string_width(string_hash_to_newline(str[0]+str[1]))),yy+ly+lineheight - 1,0)
                    draw_set_color(color_selected)
                    if( global.enabledText ) draw_text(xx+string_width(string_hash_to_newline(str[0])),yy+ly,string_hash_to_newline(str[1]))
                }
                if (str[2]!="") {  // Text after selection
                    draw_set_color(color_normal)
                    if( global.enabledText ) draw_text(xx+string_width(string_hash_to_newline(str[0]+str[1])),yy+ly,string_hash_to_newline(str[2]))
                }
            } else { // Unselected line
                draw_set_color(color_normal)
                if( global.enabledText ) draw_text(xx,yy+ly,string_hash_to_newline(string_replace_all(i.line[l],"#","\#")))
            }
        }
    }
    ...
    if (textbox_focus=i && !i.read_only) {
        a=string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[textbox_select_mouseline],1,textbox_select_mousepos),"#","\#")))
        b=(textbox_select_mouseline-i.start)*lineheight
        if (i.single_line) {
            a-=string_width(string_hash_to_newline(string_replace_all(string_copy(i.line[textbox_select_mouseline],1,i.start),"#","\#")))
            b=0
        }
